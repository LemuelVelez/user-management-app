<template>
    <div class="w-full max-w-xs p-4 bg-white shadow-md rounded-md">
      <h2 class="text-center font-semibold text-xl mb-4">Update User</h2>
      <form @submit.prevent="handleSubmit">
        ...
        <button type="submit" class="w-full p-2 bg-yellow-500 text-white rounded flex items-center justify-center">
    ...
    methods: {
      handleSubmit() {
        if (this.selectedUser.name && this.selectedUser.email) {
          this.onUpdate(this.selectedUser); // Trigger the update method passed from parent
        } else {
